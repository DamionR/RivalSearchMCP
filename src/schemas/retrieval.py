...
class WebContent(BaseModel):
    ...
    images: List[Dict[str, Any]] = Field(
        default=[], description="Images found on the page"
    )
    has_multimedia: bool = Field(
        default=False, description="Whether page contains multimedia content"
    )
...
class RetrieveResult(BaseModel):
    ...
    data: Union[List[SearchResult], WebContent, Dict[str, str], TraversalResult] = (
        Field(description="Retrieved data")
    )
    truncated: bool = Field(default=False, description="Whether content was truncated")
    remaining_bytes: int = Field(
        default=0, description="Remaining content length if truncated"
    )
    ...
    is_traversal: bool = Field(
        default=False, description="Whether this was a link traversal"
    )
