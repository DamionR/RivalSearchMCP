...
class GoogleSearchResult:
    ...
    def __post_init__(self):
        ...
        if not hasattr(self, "has_rich_snippet"):
            self.has_rich_snippet = False
        if not hasattr(self, "rich_snippet_type"):
            self.rich_snippet_type = ""
        if not hasattr(self, "estimated_traffic"):
