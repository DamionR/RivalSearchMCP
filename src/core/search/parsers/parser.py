...
from ..models.models import GoogleSearchResult
...
class GoogleSearchHTMLParser(BaseSearchParser):
        ...
        try:
            ...
            if not result_blocks:
                ...
                result_blocks = [
                    ...
                    if isinstance(block, Tag) and block.find("a", href=True)
                ]
            ...
            for position, result_block in enumerate(result_blocks, 1):
                try:
                    ...
                    search_result.search_features = self.extract_search_features(
                        result_block
                    )
                    ...
                    if search_result.search_features:
                        ...
                        rich_types = [
                            f
                            for f in search_result.search_features
                            if f.startswith("rich_snippet_")
                        ]
                        if rich_types:
                            search_result.rich_snippet_type = rich_types[0].replace(
                                "rich_snippet_", ""
                            )
